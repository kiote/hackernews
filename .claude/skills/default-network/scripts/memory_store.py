...
Usage:
    ...
    python memory_store.py quick "concept1,concept2,concept3"
...
def normalize_concept_name(name: str) -> str:
    """Normalize concept name: lowercase, strip whitespace, collapse spaces."""
    return " ".join(name.lower().strip().split())


def load_concepts() -> dict:
    """Load concept graph from file with error recovery."""
        ...
        try:
            with open(concepts_file, "r") as f:
                ...
                if not isinstance(data, dict):
                    raise ValueError("Invalid concepts file structure")
            ...
            backup_file = STATE_DIR / f"concepts.json.corrupted.{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            print(f"Warning: Corrupted concepts.json, backing up to {backup_file}")
            concepts_file.rename(backup_file)
        except Exception as e:
            print(f"Warning: Error loading concepts.json: {e}")
    return {"nodes": {}, "edges": []}


def save_concepts(data: dict):
    """Save concept graph with atomic write."""
    ensure_state_dir()
    concepts_file = STATE_DIR / "concepts.json"
...
def store_edge(source: str, target: str, relationship: str):
    ...
    source = normalize_concept_name(source)
    target = normalize_concept_name(target)
...
class ConceptBatch:
    """Context manager for batched concept operations with single save."""
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.modified and exc_type is None:
            save_concepts(self.data)
        return False

    def add_concept(self, name: str, attributes: List[str] = None) -> str:
        ...
        if name not in self.data["nodes"]:
            ...
            if attributes:
                existing = node.get("attributes", [])
                node["attributes"] = list(set(existing + attributes))
    ...
    def add_edge(self, source: str, target: str, relationship: str) -> bool:
...
def quick_save(concepts: List, edges: List[Tuple[str, str, str]] = None) -> Tuple[int, int]:
    """
    Quick save multiple concepts and edges in single operation.

    Args:
        concepts: List of concept names or (name, [attributes]) tuples
        edges: List of (source, target, relationship) tuples

    Returns:
        Tuple of (concepts_added, edges_added)
    """
    with ConceptBatch() as batch:
        for concept in concepts:
            if isinstance(concept, tuple):
                batch.add_concept(concept[0], concept[1] if len(concept) > 1 else None)
            else:
                batch.add_concept(concept)

        if edges:
            for source, target, rel in edges:
                batch.add_edge(source, target, rel)

        return batch.concepts_added, batch.edges_added
...
def main():
    ...
    if command == "store":
        ...
        if len(sys.argv) < 3:
            print("Usage: memory_store.py quick <concept1,concept2,...>")
            return
        concepts = [c.strip() for c in sys.argv[2].split(",")]
        added, _ = quick_save(concepts)
        print(f"Saved {len(concepts)} concepts ({added} new)")
